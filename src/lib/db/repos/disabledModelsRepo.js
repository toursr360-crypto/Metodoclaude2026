...
export async function disableModels(providerAlias, ids) {
  ...
  await db.transaction(async () => {
    ...
    await db.run(`INSERT INTO kv(scope, key, value) VALUES(?, ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, [SCOPE, providerAlias, stringifyJson(merged)]);
  });
}
...
export async function enableModels(providerAlias, ids) {
  ...
  await db.transaction(async () => {
    ...
    if (next.length === 0) {
      ...
      await db.run(`INSERT INTO kv(scope, key, value) VALUES(?, ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, [SCOPE, providerAlias, stringifyJson(next)]);
    }
  });
}
