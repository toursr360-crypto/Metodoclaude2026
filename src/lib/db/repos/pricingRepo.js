...
function invalidate() { cache = { value: null, expiresAt: 0 }; }

async function getUserPricing() { return await pricingKv.getAll(); }
...
export async function getPricing() {
  ...
  for (const [provider, models] of Object.entries(userPricing)) {
    ...
    else { for (const [model, pricing] of Object.entries(models)) { if (!merged[provider][model]) merged[provider][model] = pricing; } }
  }
...
export async function resetPricing(provider, model) {
  ...
  const db = await getAdapter();
  await db.transaction(async () => {
    if (!model) { await db.run(`DELETE FROM kv WHERE scope = 'pricing' AND key = ?`, [provider]); return; }
    const row = await db.get(`SELECT value FROM kv WHERE scope = 'pricing' AND key = ?`, [provider]);
    const current = row ? (parseJson(row.value, {}) || {}) : {};
    ...
    if (Object.keys(current).length === 0) {
      ...
      await db.run(`INSERT INTO kv(scope, key, value) VALUES('pricing', ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value = excluded.value`, [provider, stringifyJson(current)]);
    }
  });
  invalidate();
  return await getUserPricing();
